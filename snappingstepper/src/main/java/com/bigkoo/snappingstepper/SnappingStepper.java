...
public class SnappingStepper extends RelativeLayout implements View.OnTouchListener, ValueAnimator.AnimatorListener, ValueAnimator.AnimatorUpdateListener {
    ...
    private static long UPDATEDURATIONFAST2 = 10;//数值更新频率-快
    ...
    private void initViews(AttributeSet attrs) {
        ...
        //setOnTouchListener(this);//中间的滑动功能

        updateRunnable = new UpdateRunnable(this);
//        postDelayed(updateRunnable, 0);
    }

    private boolean canTouchLeft = true;
    private boolean canTouchRight = true;

    @Override
    public boolean onTouch(View v, MotionEvent event) {
        if (v.equals(ivStepperMinus)) {
            if (!canTouchLeft) {
        ...
        } else if (v.equals(ivStepperPlus)) {
            if (!canTouchRight) {
...
//        if (nextValue <= minValue) {
//            nextValue = minValue;
//            canTouchLeft = false;//之后可以在这里设置背景色
//            //setLeftButtonResources(R.drawable.no_jh);
//        } else {
//            canTouchLeft = true;
//            //setLeftButtonResources(R.drawable.ic_snappingstepper_minus);
//        }
//        if (nextValue >= maxValue) {
//            nextValue = maxValue;
//            canTouchRight = false;
//            //setRightButtonResources(R.drawable.no_add);
//        } else {
//            canTouchRight = true;
//            //setRightButtonResources(R.drawable.ic_snappingstepper_plus);
//        }
        value = nextValue;
        if (mode == Mode.AUTO)//AUTO模式，写数值到滑动条上
            //tvStepperContent.setText(String.valueOf(value)); //这里改动，不让更新时候直接显示到中间
            if (listener != null)
                listener.onValueChange(this, value, type);
        if (stepTouch)
...
//            postDelayed(updateRunnable, (System.currentTimeMillis() - startTime > STEPSPEEDCHANGEDURATION) ? UPDATEDURATIONFAST2 : UPDATEDURATIONFAST2);
    ...
    public void setValue(int value) {
        ...
        if (mode == Mode.AUTO)//AUTO模式，写数值到滑动条上
            tvStepperContent.setText(String.valueOf(this.value));
    }
    ...
    public void setLeftBackground(int drawable) {
        ivStepperMinus.setBackgroundColor(drawable);
    }

    public void setRightBackground(Drawable drawable) {
        ivStepperPlus.setBackgroundDrawable(drawable);
    }
     ...
     * @param resId
     */
    public void setLeftButtonVisiable(int resId) {
        ivStepperMinus.setImageResource(resId);
    }
